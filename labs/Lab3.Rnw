...
\title{2015 MFE Programming Workshop Lab 3}
\hypersetup{
  pdfkeywords={},
  pdfsubject={},
...
\verb~lubridate~.
\item You'll need to run regressions on groups. See the \verb~do~ verb in the
\verb~dplyr~ reference manual and the associated examples. Another
package called \verb~broom~ and its function \verb~tidy~ makes this
particularly simple.
\item You may need to use \verb~tidyr~ functions.
\item You'll need to use a join or a merge. Recall that \verb~dplyr~ has some
nice join functions.
\end{itemize}
